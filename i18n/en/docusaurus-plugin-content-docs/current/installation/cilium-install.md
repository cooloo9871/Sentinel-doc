...
K8s Sentinel relies on **Cilium** as the cluster CNI, and several K8s Sentinel modules require specific Cilium features to be enabled:

| K8s Sentinel feature | Required Cilium capability |
|---|---|
| **Network Topology** | Hubble flow observation (including L7) - K8s Sentinel reads the cluster-wide aggregated flows from **Hubble Relay** over gRPC |
...
| Flag | Description |
|---|---|
| `hubble.enabled=true` | **Required by K8s Sentinel**: turns on flow observation, the only source for Network Topology and Cilium policy denials |
| `hubble.relay.enabled=true` | **Required by K8s Sentinel (v0.43+)**: deploys **Hubble Relay**, which aggregates every node's flows behind one gRPC endpoint. K8s Sentinel connects to `hubble-relay` rather than reaching into each agent; without Relay there is no aggregated flow source to read |
| `kubeProxyReplacement=true` | **Required by K8s Sentinel**: Cilium's socket-level load balancer rewrites a Service address to the backend pod *before* the flow is observed, so the topology sees the real endpoint. Left to kube-proxy, the flow carries the ClusterIP and K8s Sentinel drops that edge - a VIP is not an endpoint |
...
# Hubble Relay running (the flow source K8s Sentinel reads)
...
The `kubectl get crd` output should include `ciliumnetworkpolicies.cilium.io` and `ciliumclusterwidenetworkpolicies.cilium.io` - a prerequisite for K8s Sentinel's Network Policy feature.

:::tip
K8s Sentinel assumes Cilium is installed in the `kube-system` namespace by default. If you installed it elsewhere, set the `CILIUM_NAMESPACE` environment variable accordingly when deploying K8s Sentinel.
:::
